...
(define (map proc list . lists)
  (if (pair? lists)
      (do ((lists (cons list lists) (map cdr lists))
           (acc '() (cons (apply proc (map car lists)) acc)))
          ((memv '() lists) (reverse acc)))
...
(define (string-map proc string . strings)
  (if (pair? strings)
      (list->string (apply map proc (map string->list (cons string strings))))
...
(define (vector-map proc vector . vectors)
  (if (pair? vectors)
      (list->vector (apply map proc (map vector->list (cons vector vectors))))
...
(define (string-for-each proc string . strings)
  (if (pair? strings)
      (apply for-each proc (map string->list (cons string strings)))
      (let ((length (string-length string)))
        (do ((idx 0 (+ idx 1)))
            ((= idx length))
...
(define (vector-for-each proc vector . vectors)
  (if (pair? vectors)
      (apply for-each proc (map vector->list (cons vector vectors)))
      (let ((length (vector-length vector)))
        (do ((idx 0 (+ idx 1)))
            ((= idx length))
